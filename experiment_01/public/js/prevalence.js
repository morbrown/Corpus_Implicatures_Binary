function make_slides(f) {
  ...
  slides.example1 = slide({
   ...
   slides.example2 = slide({
    ...
    start: function() {
      $(".err").hide();
      //lihne below this is likert scale generator
...
/*   button : function() {
  ...
  log_responses : function() {
    exp.data_trials.push({
      "slide_number_in_experiment" : exp.phase,
      "tgrep_id" : this.generic.Item_ID,
      "response" : [this.radio, exp.checkbox.checked],
      "sentence" : this.generic.ComparisonSentence,
    });
}, */
 ...
 slides.startExp = slide({
  ...
  slides.generateEntities = slide({
    ...
    $("input[name='number']:checked").prop('checked', false);
      ...
      // usentence = generic.OriginalSentence;
      // butnotboth = generic.ComparisonSentence;

      usentence = generic.OriginalSentence.replace(/--n((\d+)|[a-z]+)+/g, "");
      butnotboth = generic.ComparisonSentence;
      ...
      this.counter++;
      //likert scale begins here
     // $("#tableGenerator").html('<table id="tableGenerator" align="left"> </table>');

      //Trying to copy radio button js for generate entities radio buttons
      slides.generateEntities =  slide({
        name : "subj_info",
        submit : function(e){
          //ifFdata (e.preventDefault) e.preventDefault(); // I don't know what this means.
